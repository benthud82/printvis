...
$openbatches_case_cols = 'casebatch_whse, casebatch_build, casebatch_cart, casebatch_aisle, casebatch_trips, casebatch_cubicinch, casebatch_lines, casebatch_firstloc, casebatch_lastloc, casebatch_minbin, openbatches_printdatetime,casebatch_uniquelocs, casebatch_noncon';
...
$casetotetimes_cols = 'casetote_time_whse, casetote_time_build, casetote_time_cart, casetote_time_aisle, casetote_time_equipment, casetote_time_trips, casetote_time_lines, casetote_time_firstloc,'
        . 'casetote_time_lastloc, casetote_time_printdate, casetote_time_pickloctime, casetote_time_pickunittime, casetote_time_indirecttime, casetote_time_dropofftime, casetote_time_noncontime, casetote_time_inches_inneraisle, casetote_time_inches_outeraisle, casetote_time_traveltime, casetote_time_totaltime';
...
//    $querydelete5->execute();
    ...
    if ($arraycount > 0) {
        ...
        //Add to table casebatchstarttime_hist

        $sql6 = "INSERT IGNORE INTO printvis.casebatchstarttime_hist ($casestartcols) VALUES $values5";
    ...
    $batchdata = $aseriesconn->prepare("SELECT
                                                                                ...
                                                                                sum(case when LMTIER in ('C03','C07') and substr(LMLOC#,6,1) = '1' then 1 else 0 end) as PALLET_PICKS, 
                                                                                sum(case when LMTIER in ('C05', 'C06')  and substr(LMLOC#,6,1) >= '2' then 1 else 0 end) as HALFDECK_PICKS,
                                                                                sum(case when LMTIER not in ('C01', 'C02', 'C03', 'C05', 'C06', 'C07') then 1 else 0 end) as OTHER_PICKS,
...
//Pull in ship zone cutoff times
    ...
    $totedata = $aseriesconn->prepare("SELECT 
                                                                                PBWHSE,
                                                                                CASE
                                                                                ...
                                                                                END))) AS CUBIC_INCH,
                                                                                COUNT(*) AS LINE_COUNT,
                                                                                    (SELECT PDLOC# FROM HSIPCORDTA.NOTWPD T WHERE T.PDCART = PBCART and PDBIN# = MIN(A.PDBIN#)) as FIRSTLOC,
                                                                                    (SELECT PDLOC# FROM HSIPCORDTA.NOTWPD T WHERE T.PDCART = PBCART and PDBIN# = MAX(A.PDBIN#)) as LASTLOC,
                                                                                    (SELECT MIN(A.PDBIN#) FROM HSIPCORDTA.NOTWPD T WHERE T.PDCART = PBCART and PDBIN# = MIN(A.PDBIN#)) as MINBIN,
                                                                                SUM(CASE
                                                                                ...
                                                                                SUM(CASE
                                                                                    ...
                                                                                    ELSE 0
                                                                                END) AS OTHER_PICKS,
                                                                                count(DISTINCT PDLOC#) as UNIQUE_LOCS,
                                                                                sum(case when PBBOXV * .061024 > 10368 then 1 else 0 end) as COUNT_NONCON
                                                                            FROM
                                                                                HSIPCORDTA.NOTWPD A
                                                                                     LEFT JOIN
                                                                            ...
                                                                            WHERE
                                                                                PDWHSE = $whsesel AND PDBXSZ = 'CSE'
                                                                                    ...
                                                                                    AND PBSHPC <> 'CPL'
                                                                                 --  and PBCART = 15643
                                                                            GROUP BY PBWHSE , CASE
                                                                                WHEN PBWHSE = 3 AND PDLOC# >= 'W400000' THEN 2
                                                                                ELSE 1
                                                                            END , PBCART , PBPTJD , PBPTHM , SUBSTR(PDLOC#, 1, 3)
                                                                            ORDER BY PBWHSE , CASE
                                                                                WHEN PBWHSE = 3 AND PDLOC# >= 'W400000' THEN 2
                                                                                ELSE 1
                                                                            END , PBCART , PBPTJD , PBPTHM , SUBSTR(PDLOC#, 1, 3)");
    $totedata->execute();
    ...
    @LOWSTART_X:=(SELECT 
        ...
        WHERE
            CONCAT(casebatch_aisle, 'L') = casemap_loc
                AND casemap_whse = casebatch_whse) AS LOWSTART_X,
    @LOWSTART_Z:=(SELECT 
            casemap_zcoor
        FROM
            printvis.pickprediction_casemap
        WHERE
            CONCAT(casebatch_aisle, 'L') = casemap_loc
                AND casemap_whse = casebatch_whse) AS LOWSTART_Z,
    @HIGHSTART_X:=(SELECT 
            pickprediction_xcoor
        FROM
            printvis.pickprediction_casemap
        WHERE
            CONCAT(casebatch_aisle, 'H') = casemap_loc
                AND casemap_whse = casebatch_whse) AS HIGHSTART_X,
    @HIGHSTART_Z:=(SELECT 
            casemap_zcoor
        FROM
            printvis.pickprediction_casemap
        WHERE
            CONCAT(casebatch_aisle, 'H') = casemap_loc
                AND casemap_whse = casebatch_whse) AS HIGHSTART_Z,
    @BRIDGESTART_X:=(SELECT 
            pickprediction_xcoor
        FROM
            printvis.pickprediction_casemap
        WHERE
            CONCAT(casebatch_aisle, 'B') = casemap_loc
                AND casemap_whse = casebatch_whse) AS BRIDGESTART_X,
    @BRIDGESTART_Z:=(SELECT 
        ...
        WHERE
            CONCAT(casebatch_aisle, 'B') = casemap_loc
                AND casemap_whse = casebatch_whse) AS BRIDGESTART_Z,
    @LASTLOC_X:=(SELECT 
            pickprediction_xcoor
        FROM
            printvis.pickprediction_casemap
...
ORDER BY casebatch_cart , casebatch_minbin");
    ...
    foreach ($opentotedataarray as $key => $value) {
        ...
        if ($openbatchcount == 0) {  //first aisle on batch.  what is distance from CSTART to first location
            ...
            $openbatchcount = 0;
        } elseif ($casebatch_cart !== $opentotedataarray[$key + 1]['casebatch_cart'] && $openbatchcount == 0) {//if new batch is on next line and this is first line of batch, go from CSTART to Location the location back to CSTOP
            $outeraisle_min = abs($FIRSTLOC_X - $cstart_xcoor) + abs($FIRSTLOC_Z - $cstart_zcoor);
            $outeraisle_min += abs($LASTLOC_X - $cstop_xcoor) + abs($LASTLOC_Z - $cstop_zcoor);
            $openbatchcount = 0;
        } elseif ($casebatch_cart !== $opentotedataarray[$key + 1]['casebatch_cart'] && $casebatch_cart !== $opentotedataarray[$key - 1]['casebatch_cart']) {
            $outeraisle_min = abs($FIRSTLOC_X - $cstart_xcoor) + abs($FIRSTLOC_Z - $cstart_zcoor);
            $outeraisle_min += abs($LASTLOC_X - $cstop_xcoor) + abs($LASTLOC_Z - $cstop_zcoor);
        } elseif ($casebatch_cart !== $opentotedataarray[$key + 1]['casebatch_cart']) {//if new batch is on next line do normal calc then add distance from last location and go back to CSTOP
            // $outeraisletravel = abs($opentotedataarray[$key - 1]['LASTLOC_X'] - $opentotedataarray[$key - 1]['AISLESTART_X']) + abs($opentotedataarray[$key - 1]['LASTLOC_Z'] - $opentotedataarray[$key - 1]['AISLESTART_Z']);  //Previous last loc to previous start.
            //$outeraisletravel += abs($AISLESTART_X - $opentotedataarray[$key - 1]['AISLESTART_X']) + abs($AISLESTART_Z - $opentotedataarray[$key - 1]['AISLESTART_Z']);
            //$outeraisletravel += abs($FIRSTLOC_X - $AISLESTART_X) + abs($FIRSTLOC_Z - $AISLESTART_Z);
            $outeraisle_min += abs($LASTLOC_X - $cstop_xcoor) + abs($LASTLOC_Z - $cstop_zcoor);
            ...
            if ($whsesel == 3 && $building == 2 && substr($opentotedataarray[$key - 1]['casebatch_lastloc'], 0, 3) == 'W49') {
        ...
        $data_totetimes[] = "($casebatch_whse, $casebatch_build, $casebatch_cart, '$casebatch_aisle', '$caseequip_equipment', '$casebatch_trips', $casebatch_lines, '$casebatch_firstloc', '$casebatch_lastloc', '$openbatches_printdatetime', '$calc_picktimeloc', '$calc_picktimeunit', '$calc_picktimeindirect', '$calc_picktimedropoff', '$calc_picktimenoncon', $caseaisle_inches_inner, $outeraisle_min,  '$calc_taveltime', '$totaltime')";
    }
...
//        }
    ...
    if (!is_null($batchcompletedata)) {
        ...
        $sql4 = "INSERT INTO printvis.casebatches_time ($casebatchtimes_cols) VALUES $values4 ON DUPLICATE KEY UPDATE casebatches_equipment=VALUES(casebatches_equipment), casebatches_time_kiosk=VALUES(casebatches_time_kiosk), casebatches_time_batch=VALUES(casebatches_time_batch), casebatches_time_complete=VALUES(casebatches_time_complete), casebatches_time_short=VALUES(casebatches_time_short), casebatches_time_pickloc=VALUES(casebatches_time_pickloc), casebatches_time_unit=VALUES(casebatches_time_unit), casebatches_time_indirect=VALUES(casebatches_time_indirect), casebatches_time_dropoff=VALUES(casebatches_time_dropoff), casebatches_time_noncon=VALUES(casebatches_time_noncon), casebatches_time_aisletravel=VALUES(casebatches_time_aisletravel), casebatches_time_startstop=VALUES(casebatches_time_startstop),  casebatches_time_final=VALUES(casebatches_time_final) ";
